...
const UserProfilePage = ({ params }: Params) => {
  interface ObjectId {
    _id: string;
  }
  ...
  interface User {
    _id: string; // assuming this will be converted to a string
    name: string;
    email: string;
    password: string;
    phone: string;
    ...
    isDeleted: boolean;
    isPremium: boolean;
    following: string[]; // assuming the following/followers will be an array of strings (ObjectId converted to strings)
    followers: string[];
  }
  ...
  const { data: recipe, isLoading: recipeLoading } =
    useGetRecipeByEmailQuery(email,{pollingInterval: 1000 });
    ...
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 md:p-8">
      ...
      <div className="col-span-1 md:col-span-3 grid grid-rows-2 gap-4">
        ...
        <div className="bg-gray-100 p-4 rounded-lg shadow-md">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mt-4">
            {userRecipe?.map((recipeItem: Recipe) => (
              <div
              ...
              >
                ...
                />
                <p className="text-sm">{recipeItem.title || "name"}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="col-span-1 bg-gray-100 p-4 rounded-lg shadow-md">
        ...
        <div className="grid grid-cols-1 gap-4 mt-4">
          {allRecipes?.map((recipeItem: Recipe) => (
            <div
            key={recipeItem._id.toString()}
